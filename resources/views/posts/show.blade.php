    ...
    <div class="row">
        ...
        <div class="span8">
            ...
            <div class="row">
                ...
                <div class="span10">
                    ...
                    @if(is_null($post->reciever))
                        <div class="jumbotron col-lg-1 ">
                            <h4>Kandidati:</h4>
                            @if(count($post->candidates) == 0)
                                <h4>Nema kandidata</h4>
                                @endif
                            @foreach($post->candidates as $candidate)
                                <p><strong> <a class="btn" href="/posts/{{$post->id}}/pick/{{$candidate->id}}">Odaberi {{$candidate->name}} {{$candidate->lastname}}</a> </strong></p>
                            @endforeach
                            @if(count($post->candidates) == 0)
                               @else
                                <h4><strong> <p><a class="btn" href="/posts/{{$post->id}}/pickRandom">Odaberi nasumično</a></p></strong></h4>
                            @endif
                        ...
                        <div class="row">
                        <div class="span8">
                            <h4><strong>  Primatelj : {{$post->reciever->name}} {{$post->reciever->lastname}}</strong></h4>
                        </div>
                        </div>
        ...
        <div class="container">
            <div class="row">
                ...
                <form method="POST"  action="/comment/{{$post->id}}"  id = comment{{$post->id}}  >
                    {{ csrf_field() }}
